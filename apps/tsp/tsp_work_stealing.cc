...
{
   public:
      ThreadData(int tid, int NUM_CITIES);
      ~ThreadData();
   ...
   ThreadData::ThreadData(int tid, int NUM_CITIES)
   : _tid(tid)
...
void* threadWork(void* targ)
{
   ...
   // use shared queue and steal work here
   while (1)
   {
#if DEBUG
      ...
      int items_to_pop = CHUNK_SIZE;

      pthread_mutex_lock(&shared_queue_lock);
      while (!partial_tour_vec.empty() && items_to_pop > 0 && work_queue.size() < CHUNK_SIZE) {
         // Pop the last element from partial_tour_vec
         PartialTour* tour = partial_tour_vec.back();
         partial_tour_vec.pop_back();
         
         // Push it into the current work queue
         work_queue.push_back(tour);
         // Decrement the counter for number of items to pop
         items_to_pop--;
      }
...
int main(int argc, char** argv)
{
   ...
   for (int i = 0; i < NUM_THREADS; i++)
      thread_data[i] = new ThreadData(i, NUM_CITIES);

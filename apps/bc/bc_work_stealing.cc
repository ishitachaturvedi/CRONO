...
void* do_work(void* args)
{
   ...
   //printf("tid:%d start %d stop %d iter %d\n",tid,i_start,i_stop,arg->iter);
   ...
   //    } else {
   //       double start_d = 0;
   //       double stop_d = N_d * first_thread_work;
   //       i_start = (int)start_d;
   //       i_stop = (int)stop_d;
   //    }
   // } else {
   //    // Remaining work distributed among the rest of the threads
   //    double start_d = N_d * first_thread_work + (tid_d - 1) * (N_d * (1 - first_thread_work) / (P_d - 1));
   //    double stop_d = N_d * first_thread_work + tid_d * (N_d * (1 - first_thread_work) / (P_d - 1));
   //    i_start = (int)start_d;
   //    i_stop = (int)stop_d;
   // }
   ...
   //printf("ENTER i_start %d N_d %d CHUNK_SIZE %d\n",i_start,N_d,CHUNK_SIZE);
   while(iterate) {
      pthread_mutex_lock(&lock);
      i_start = iter;
      iter += CHUNK_SIZE;
      ...
      i_stop = (i_start + CHUNK_SIZE < N) ? i_start + CHUNK_SIZE : N;

      // Process this chunk
      for (int j = i_start; j < i_stop; j++) {
         avg[j] = 0; // Initialize avg[j]
         for (int k = 0; k < P; k++) {
               avg[j] += sigma[k][j];
         }
         avg[j] /= P;

...
// Global Variables
pthread_mutex_t shared_queue_lock;  // Lock for shared work queue
int current_row = 0;                // Current row to be processed
pthread_mutex_t locks[4194304];    // Locks for each vertex
int local_min_buffer[1024];
int global_min_buffer;
int Total = 0;
int terminate = 0;                 // Work termination flag
int P_global = 256;
int *edges;                        // Degree of a given vertex
int *exist;                        // Whether vertex exists in the graph
int *temporary;
int largest = 0;
int avg_deg = 0;
...
void* do_work(void* args) {
   ...
   while (1) {
      ...
      // Determine the end row for this chunk
      ...
      stack[ptr] = startRow;
      int disjoint = startRow;
      ptr++;

      for (int vv = startRow; vv < endRow; vv++) {
         if(ptr>0)
         {
            ptr--;
            v = stack[ptr];
